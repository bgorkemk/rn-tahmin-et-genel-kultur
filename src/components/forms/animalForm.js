  ...
  calculateScores() {
    ...
    if (this.state.answer_given == 0) {
      ...
      Alert.alert(
        ...
        [
          { text: 'Devam!', onPress: () => { Actions.animalForm({ QuestionNumber: this.state.question_number, Score: this.state.score, Nick: this.state.nick, Data: this.state.array, askedQuestions: this.state.askedQuestions }) } }
        ],
        { cancelable: false }
      )
    }
